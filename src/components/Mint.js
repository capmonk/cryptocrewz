...
const Mint = () => {
  ...
  const { setAccount, account, setCode, setEmail, setCount } =
    useSharedUserData();
  ...
  const GetUserData = async (walletType, account) => {
    ...
    account.isWhitelisted = (
      await isWhitelisted(account.address)
    ).iswhitelisted;
    ...
    <div>
      ...
      <div className="mint-container w-96">
        <div
          ...
          className="col-7 flex flex-col items-center"
        >
                ...
                {account.address ? (
                    ...
                    {account.address ? (
                      <div className="flex flex-col items-center text-xs font-light">
                        <div>
                          ...
                          <a
                            href={
                              process.env.REACT_APP_EXPLORER_ADDRESS +
                              "/address/" +
                              account.address
                            }
                            target="_blank"
                  ...
                  <div>
                    ...
                    <button
                      ...
                      type="button"
                      className={
                        "border-green-440 hover:bg-green-400 py-2 w-72 px-9 pr-12 uppercase italic text-xl border border-solid rounded-full"
                      }
          ...
          <div className="h-64 w-96 mt-2">
            {!contractData.address &&
            !contractData.whitelistSaleIsActive &&
      ...
      {walletModal ? (
        <>
          <div className="left-0 top-0 fixed h-screen w-screen">
            <div
              className="absolute left-0 top-0  h-screen w-screen bg-black opacity-90"
              onClick={closeWalletModal}
            ></div>
            <div className="absolute left-0 top-0  h-screen w-screen grid justify-items-center items-center pointer-events-none">
              <div
                className="flex flex-col items-center p-4 w-80 bg-opacity-50 rounded-3xl  pointer-events-auto"
              ...
              >
                ...
                {true ? (
                  <>
                    <button
                      ...
                      className="flex flex-row items-center justifty-spacebetween bg-black bg-opacity-40 hover:bg-gray-800 w-full h-12 p-4 mb-4 h-16 rounded-3xl font-light"
                    >
                      <img
                        className="h-6 ml-2 mr-5"
                        alt="Metamask"
                        src={metamask}
                      ></img>
                      Metamask
                ...
                <button
                  ...
                  className="flex flex-row items-center justifty-spacebetween bg-black bg-opacity-40 hover:bg-gray-800 w-full h-12 mb-4 p-4 h-16 rounded-3xl font-light"
                >
                  <img
                    className="h-8 ml-2 mr-5"
                    alt="Facebook"
                    src={walletlink}
                  ></img>
                  <div>Continue with Coinbase Wallet</div>
                </button>

                <button
                  onClick={() => {
                    connectWallet("google");
                  }}
                  className="flex flex-row items-center justifty-spacebetween bg-black bg-opacity-40 hover:bg-gray-800 w-full h-12 p-4 mb-4 h-16 rounded-3xl font-light"
                >
                  <img
                ...
                <button
                  onClick={() => {
                    connectWallet("facebook");
                  }}
                  className="flex flex-row items-center justifty-spacebetween bg-black bg-opacity-40 hover:bg-gray-800 w-full h-12 p-4 h-16 rounded-3xl font-light"
                >
                  <img
                    className="h-8 ml-2 mr-5"
                    alt="Facebook"
